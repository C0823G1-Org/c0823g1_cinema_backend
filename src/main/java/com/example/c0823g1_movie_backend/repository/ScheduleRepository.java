...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
    ...
    @Query(value = "select st.id,st.schedule_time from schedule_time st join schedule s on st.id =" +
            "(select schedule_time_id from schedule where movie_id = :movieId and date = :dateTime ) ", nativeQuery = true)
